...
function renderSkill(skillId, opts = {}) {
  ...
  app.innerHTML = `
    <div class="card">
      ...
      ${skill.drills.map(d => {
        ...
        const p = getOrInitDrillProgress(state, d, persist);
  ...
  skill.drills.forEach(d => {
    ...
    if (down) down.onclick = () => {
      const p = getOrInitDrillProgress(state, d, persist);
      setDrillBpm(state, d, (p.bpm || cfg.start) - (cfg.step || 5), persist);
      renderSkill(skillId, opts);
    };
    ...
    if (clean) clean.onclick = () => {
      markCleanRep(state, d, persist);
      renderSkill(skillId, opts);
    };

    if (sloppy) sloppy.onclick = () => {
      markSloppyRep(state, d, persist);
      renderSkill(skillId, opts);
    };

    if (reset) reset.onclick = () => {
      resetDrillProgress(state, d, persist);
      renderSkill(skillId, opts);
    };
  });
